...
class Appbyme
{
    ...
    public static function getPostSign($key, $type, $status)
    {
        ...
        $defConfig    = Appbyme::getJumpSetting();
        $defWz        = $defConfig['wzurl'];
        $defPostSign  = array(
            'forumdisplay_thread' => array(
                ...
                'weizhan' => '<img src="mobcent/app/web/images/mobile/mobile-attach-1.png" alt="weizhan" align="absmiddle">',
                'weapp' => '<img src="mobcent/app/web/images/mobile/mobile-attach-1.png" alt="weapp" align="absmiddle">',
            ),
            'viewthread_post'     => array(
                ...
                'weizhan' => sprintf('<a href="%s" target="_blank">%s</a>', $defWz, self::t('来自微站')),
                'weapp' => sprintf('%s', self::t('来自小程序')),
            ),
            'viewthread_avatar'   => array(
                ...
                'weizhan' => '<div><img style="margin: -145px 0 0 100px;" src="mobcent/app/web/images/mobile/mobile-weizhan.png" /></div>',
                'weapp' => '<div><img style="margin: -145px 0 0 100px;" src="mobcent/app/web/images/mobile/mobile-weapp.png" /></div>',
            )
        );
        ...
        $matches      = array();
        preg_match_all('/\[title_(all|android|apple|weizha|weapp)\](.*?)\[\/title_\1\]/', $postSignText, $matches, PREG_SET_ORDER);
        foreach ($matches as $matche)
        {
            if ($matche[1] == 'all')
            {
                $postSigns['android'] = $postSigns['apple'] = $postSigns['weizhan'] = $postSigns['weapp'] = $matche[2];
                break;
            }
            else if ($matche[1] == 'android' || $matche[1] == 'apple' | $matches[1] == 'weizhan'|$matche[1]=='weapp')
            {
    ...
    public static function getJumpDefaultSetting()
    {
        return array(
            'wzurl'   => self::getWzDefaultUrl(),
            'article' => 1,
            'thread'  => array(0, 1, 2, 3, 4, 5, 127),
            'user'    => 1,
            'plugin'  => 0,
            'all'     => 1,
            'wzjump'  => 1,
    ...
    protected static function getWzDefaultUrl()
    {
        $forumKey = self::getAppbymeConfig('app_forumkey');
        if (empty($forumKey))
        {
            return '';
        }
        $forumKey = $forumKey['cvalue'];

        return 'http://' . $forumKey . '.xiaoyun.com';
    }
}
