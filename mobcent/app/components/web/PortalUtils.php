...
class PortalUtils {
     ...
     * @return mixed
     */
    public static function getAidByTag($tag)
    {
        $res = DbUtils::getDzDbUtils(true)->queryFirst('SELECT aid FROM %t WHERE tag=%d LIMIT 0,1', array('portal_article_title', $tag));
        return $res;
    }
     ...
     * @return mixed
     */
    public static function getAidByLimit($max, $arr, $aid)
    {
        $arr[] = $aid;
        $maxid = DbUtils::getDzDbUtils(true)->queryFirst('SELECT aid FROM %t ORDER BY dateline DESC LIMIT 0,1', array('portal_article_title'));

        $vel = array();
        for($i = 0; $i < 15; $i++) {
            $rand = rand(1,$maxid);
            if(!in_array($rand, $arr) && !in_array($rand, $vel))
                $vel[] = $rand;
        }
        $res = DbUtils::getDzDbUtils(true)->queryAll('SELECT aid FROM %t WHERE aid NOT IN(%n) AND aid IN (%n) LIMIT 0,%d', array('portal_article_title', $arr, $vel, $max));
        return $res;
    }
}
