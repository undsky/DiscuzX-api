...
class ImageUtils {
    ...
    public static function getThumbImageEx($image, $timeout=15, $getImageInfo=false, $inBackgroud=false, $force=false) {
        ...
        global $_G;
        $thumbConfig =  Yii::app()->params['mobcent']['thumb'];
        if ($thumbConfig['enable'] && strpos($image, $thumbConfig['domain']) !== false) {
            $str = str_replace("{width}",$config['imageSmallLength'],$thumbConfig['str']);
            $res['image'] = $image.$str;
            $attachFile = $image;
            ...
            $imageInfo = self::getImageInfo($attachFile);
    ...
    private static function getImageInfo($image) {
        ...
        $cacheInfo =  Yii::app()->params['mobcent']['cache']['imageInfo'];
        $info = array();
        if($cacheInfo['enable']==1){
            $info =  unserialize(Yii::app()->cache->get('image_'.$image));
        }
        if(!$info){
            $info = self::getImageInfoNotCache($info);
            if($cacheInfo['enable']==1){
                Yii::app()->cache->set('image_'.$image,serialize($info),$cacheInfo['expire']);
            }
        }
    ...
    private static function getImageInfoNotCache($image){
