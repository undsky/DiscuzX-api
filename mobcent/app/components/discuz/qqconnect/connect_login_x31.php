...
$QQLoginBaseUrl = $callback . "/connect.php?appbyme=1";
$_G['connect']['callback_url'] = $callback . '/connect.php?appbyme=1&mod=login&op=callback';
...
if ($op == 'init') {
    ...
    if (!$_G['setting']['connect']['oauth2']) {
        ...
        echo '<script type="text/javascript">setTimeout("window.location.href=\'' . $QQLoginBaseUrl . '&openid=' . $conopenid . '&oauth_token=' . $conuintoken . '&receive=yes&mod=login&op=callback&code=&state&referer='.urlencode($_GET['referer']) . '\'", 1)</script>';
        exit;
    }
