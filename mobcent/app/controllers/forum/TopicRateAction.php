...
{
    public function run($tid, $pid, $referrer = '')
    {
        ...
        // }
        $this->_viewRate($res, $tid, $pid, $referrer);
    }
    ...
    private function _viewRate($res, $tid, $pid, $referrer)
    {   
        $res = $this->_checkRate($res, $tid, $pid);
...
HTML;
    ...
    private  function _renderTemplates($tid, $pid, $errorMsg='')
    {
        ...
        $TopicPostInfo = ForumUtils::getTopicPostInfo($tid);
        $userInfo     = array(
            'username' => $TopicPostInfo['author'],
            'avatar'   => UserUtils::getUserAvatar($TopicPostInfo['authorid']),
        );
