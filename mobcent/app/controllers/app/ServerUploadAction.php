        ...
        if ($type == 'add_certfile_apns') {
            ...
            if (UploadUtils::makeBasePath($path)) {
                if (!empty($_FILES) && count($_FILES) && is_uploaded_file($_FILES['file']['tmp_name']) && !$_FILES['file']['error']) {
                    ...
                    //把证书数据保存在数据库
                    DbUtils::getDzDbUtils(true)->insert('appbyme_config', array('ckey' => 'discuz_ios_certfile_path', 'cvalue' => $certfileAPNs),false,true);

                    DbUtils::getDzDbUtils(true)->insert('appbyme_config', array('ckey' => 'discuz_ios_certfile_info', 'cvalue' => file_get_contents($_FILES['file']['tmp_name'])),false,true);
