...
class TopicListAction extends CAction {
    ...
    public function run() {
		...
		global $_G;
        $uid = $_G['uid'];
		$data = HomeBlack::getHomeBlack($uid, $viewUid);
		if ($uid <= 0 || count($data) > 0) {
			$res = WebUtils::makeErrorInfo_oldVersion($res, 'mobcent_error_user_index_err');
			$res['rs'] = 1;
		} else {
        	$res = $this->_getTopicList($res, $type, $idType, $viewUid, $page, $pageSize);
		}
...
class UserTopicInfo extends DiscuzAR {
    ...
    public static function getTopicsByUid($uid, $page, $pageSize) {
        ...
        return DbUtils::getDzDbUtils(true)->queryColumn('
            ...
            LIMIT %d, %d
            ',
            array('forum_thread', $uid,$sql, $pageSize * ($page - 1), $pageSize)
        );
    }
