...
class UserInfoAction extends CAction {
    ...
    public function run() {
        ...
        $data = HomeBlack::getHomeBlack($uid, $puid);
		if ($uid <= 0 || count($data) > 0) {
			$res = WebUtils::makeErrorInfo_oldVersion($res, 'mobcent_error_user_index_err');
			$res['body']['repeatList'] = array();
			$res['rs'] = 1;
    		$res['is_black'] = (int) UserUtils::isBlacklist($uid, $puid) ? 1 : 0;
			$res['topic_num'] = 0;
            $res['reply_posts_num'] = 0;
            $res['follow_num'] = 0;
            $res['friend_num'] = 0;
    ...
    private function _getPersonalDataInfo($puid, $space) {
        ...
        if ($_G['uid'] == $space['uid'] || $_G['group']['allowviewip']) {
            foreach ($_G['cache']['profilesetting'] as $fieldid => $field) {
                if (!$field['available'] || $field['invisible'] || in_array($fieldid, array('birthmonth', 'birthyear', 'resideprovince', 'birthprovince', 'residecommunity', 'birthcommunity'))) {
                    continue;
                }
                if($fieldid=='qq'){
                    $val = profile_show('qqnumber', $space);
                }else{
                    $val = profile_show($fieldid, $space);
                }
                if ($field['formtype'] == 'file' && $val) {
                    $val = WebUtils::getHttpFileName(getglobal('setting/attachurl') . './profile/' . $val);
                    //                            $val = '<span><a href="' . $imgurl . '" target="_blank"><img src="' . $imgurl . '"  style="max-width: 500px;" /></a></span>';
                }
                $profiles[] = array('type' => $fieldid, 'title' => $field['title'], 'formtype'=>$field['formtype'],'data' => WebUtils::emptyHtml($val));}
        } else {
            foreach ($_G['cache']['profilesetting'] as $fieldid => $field) {
