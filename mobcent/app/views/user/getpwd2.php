...
.abtn_box .toupiao{background-color: #5bcdca;}
    ...
    <title><?php echo WebUtils::lp('user_getpwd_info_app_goback') ?></title>
    <body>
 ...
 <div class="per_margintb step2 form_item">
        ...
        $(function(){
            $('.button2').click(function () {
                if ($('#password').val() == '') {
                    alert('<?php echo WebUtils::lp('user_getpwd_info_null_failture') ?>');
                    return false;
                }
                if($('#password').val() != $('#password1').val()) {
                    alert('<?php echo WebUtils::lp('user_getpwd_info_confirm_pwd_error') ?>');
                    return false;
                }
                $.ajax({
                    type: "POST",
                    url: "<?php echo Yii::app()->createAbsoluteUrl('user/getpwd', array('act' => 'checkpwd')) ?>",
