...
class DzForumPost extends DiscuzAR {
    ...
    public static function transPostContentToHtml($post) {
        ...
        // 处理附件
        $_G['forum_attachpids'] = $_G['forum_attachtags'] = array();
        $_G['tid'] = $post['tid'];
        ...
        if (empty($postlist)) {
            ...
            foreach($postlist as $pid => $post) {
                // 取出没有插入的附件
                //别问我为什么,我的内心很崩溃- -.
                if(isset($_GET['bbcode']) && $_GET['bbcode'] && !empty($post['imagelist'])) {
                    require_once libfile('function/threadsort');
                    $threadsortshow = threadsortshow($_G['thread']['sortid'], $_G['tid']);
                    foreach($threadsortshow['sortaids'] as $v) {
                        $unset = array_search($v, $post['imagelist']);
                        unset($post['imagelist'][$unset]);
                    }
                }
        ...
        if ($post['first'] && $_G['forum_threadpay']) {
            $postlist[$pid]['message'] = WebUtils::t('本主题需向作者支付 '.$_G['forum_thread']['price']).$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title'].WebUtils::t(' 才能浏览');
        }
